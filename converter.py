...
class labelme2coco:
    ...
    def get_coco_from_labelme_folder(
        self, labelme_folder: str, coco_category_list: List = None
    ) -> Coco:
        """
        Args:
    ...
    def convert(self, input_dir: str, output_dir: str, train_split_rate: float = 1):
        """
        Args:
        ...
        for dirpath, dirnames, filenames in os.walk(input_dir):
            if any(filename.endswith(".json") for filename in filenames) and any(
...
class labelme2yolo:
    ...
    def json2df(self, annotations_json, path, type=""):
            ...
            if os.path.isdir(file_path):
                for sub_path in glob.glob(os.path.join(file_path, "*")):
                    if "images/train" in sub_path:
                        ...
                        break
                    elif os.path.isfile(sub_path) and any(
                        sub_path.lower().endswith(ext) for ext in image_extensions
                    ):
                        image_dir = file_path
                        break
        ...
        if self.dataset is not None:
            ...
            self.dataset.df = pd.concat(
                [self.dataset.df, dataset.df], ignore_index=True
            )
        else:
            self.dataset = dataset
    ...
    def convert(
        ...
        """
        self.extract_labelme(
            labelme_folder=input_dir, train_split_rate=train_split_rate
        )
        add_path = []
        ds = self.dataset
        ...
        if copy_images:
            ...
            for i in range(len(add_path)):
                Path(path_dict["image_path"] + add_path[i]).mkdir(
        ...
        output_file_paths = []
        pbar = tqdm(
            desc="Converting labelme annotations to YOLO format",
